...
// helper functions
// ----------------------
Graph_lib::Point find_point_window(Graph_lib::Shape& s, const Graph_lib::Point& up)
{

    auto point = std::upper_bound(s.begin(), s.end(), up,
        [](const Graph_lib::Point& up, const Graph_lib::Point& pred)
        {
            return (up.x < pred.x&& up.y < pred.y);
        });
    return point != s.end() ? Graph_lib::Point(point->x, point->y) : Graph_lib::Point{ 400,400 };
...
std::vector<Point> calculateGraphPointsF(Fct fc, double r1, double r2, Point orig, int count, double xscale, double yscale)
{
    std::vector<Point> points;
    double dif = (r2 - r1) / count;
    double r = r1;
    for (int n = 0; n < count; n++) 
    {
        points.push_back(Point(orig.x + (r * xscale), orig.y - (fc(r) * yscale)));
        r += dif;
    }
    return points;
}
...
std::ostream& operator<<(std::ostream& o, Graph_lib::Point p)
{
    o << "Point" << '{' << p.x << ',' << p.y << '}' << '\n';
    return o;
}

std::ostream& operator<<(std::ostream& o, Graph_lib::Text txt)
{
    o << txt.label() << " ," << txt.point(0) << '\n';
    return o;
}


std::string double_to_string(double d)
// The precision is 2 digits after decimal point
{
    std::string label = to_string((long int)(d));
    long int after_decimalpoint = (d * 100 - (long int)d * 100);
    label.push_back('.');
    std::string ad = to_string(after_decimalpoint);
    label.push_back(ad[0]);
    label.push_back(ad[1]);
    return label;
}
...
FunctionControler::FunctionControler(Fct fc, double r1, double r2, Point orig, int count, double xscale, double yscale)
    : f{ fc }, r{ r1,r2 }, po{ orig }, c{ count }, xs{ xscale }, ys{ yscale }
{
    points = std::move(calculateGraphPointsF(f, r[0], r[1], po, c, xs, ys));

}

void FunctionControler::set_range(double r1, double r2)
{ 
    r[0] = r1;
    r[1] = r2;  
    points = std::move(calculateGraphPointsF(f, r[0], r[1], po, c, xs, ys));
...
void FunctionControler::set_scale(double xscale, double yscale)
{
    xs = xscale;
    ys = yscale;
    points = std::move(calculateGraphPointsF(f, r[0], r[1], po, c, xs, ys));
...
void FunctionControler::set_fct(Fct fc)
{
    f = fc;
    points = std::move(calculateGraphPointsF(f, r[0], r[1], po, c, xs, ys));
}
...
Bars::Bars(std::initializer_list<double> list, Point p, double xscale, double yscale)
{
    for (auto l : list)
        v.push_back(l);
    double x = p.x;
    ...
    {
        label = double_to_string(v.at(n));
        pairs.push_back(std::make_pair(Rectangle({ int(x), p.y - int(v.at(n) * yscale) }, xscale + 1, v.at(n) * yscale),
            Text(Point{ int(x) - 1, p.y - int(v.at(n) * yscale) }, label)));
        x += xscale;
    }
}

void Bars::attach_owned(Simple_window& w)
{
    for (auto& p : pairs)
    {
        w.attach(p.first);
...
Bars2::Bars2(std::vector<std::pair<double,double>>& vp, Point p, double xscale, double yscale)
: v_data(vp)
{    
    double x = p.x;

    for (int n = 0; n < v_data.size(); n++)
    {
        v_rectangles.push_back(Rectangle({ int(x), p.y - int(yscale * v_data.at(n).second) }, xscale + 1, v_data.at(n).second * yscale));
        x += xscale;
    }
    for (int n = 0; n < v_data.size(); n++)
    {
        vp_texts.push_back(std::make_pair(Text(Point(sw(v_rectangles[n]).x, sw(v_rectangles[n]).y+20), double_to_string(v_data[n].first)), Text(nw(v_rectangles[n]), double_to_string(v_data[n].second))));
    }
}


void Bars2::attach_owned(Simple_window& w)
{
    for (auto& r : v_rectangles)
        w.attach(r);
    for (auto& p : vp_texts)
    {
        w.attach(p.first);
...
// Bars are placed within the Box boundaries
Bar::Bar(std::initializer_list<double> list, Point p, int width, int height, double xscale, double yscale)
    :Box(p, width, height, 0), xs{xscale}, ys{yscale}
{
    for (auto l : list)
        v.push_back(l);
    double xdif = (double)width / v.size();
    double x = p.x;
    std::string label;
    for (int n = 0; n < v.size(); n++)
    {
        label = to_string((long int)v.at(n));
        label.push_back('.');
        int after_decimalpoint = (v.at(n) * 100 - (int)v.at(n) * 100);
        std::string ad = to_string(after_decimalpoint);
        label.push_back(ad[0]);
        label.push_back(ad[1]);
        pairs.push_back(std::make_pair(Rectangle({ int(x), p.y + (height - int(v.at(n) * yscale)) }, xdif + 1, v.at(n) * yscale),
            Text(Point{ int(x) + 1, p.y + (height - int(v.at(n) * yscale)) }, label)));
        x += xdif;
    }
}
    //ydif = v.at(n) / max;
...
void Bar::attach_owned(Simple_window& w)
{
    w.attach(*this);
    for (auto & p : pairs)
    {
...
// Bars are placed within the Box boundaries, 
// using unique_ptr, for practice
Barv::Barv(std::initializer_list<double> list, Point p, int width, int height, double xscale, double yscale)
    :Box(p,width,height,0), xs{ xscale }, ys{ yscale }
{
    for (auto l : list)
        v.push_back(l);
    ...
    {        
        ...
        pairs.push_back(std::make_unique<std::pair<Rectangle,Text>>(std::make_pair(Rectangle({ int(x), p.y + (height - int(v.at(n) * yscale)) }, xdif + 1, v.at(n) * yscale),
            Text(Point{ int(x) + 1, p.y + (height - int(v.at(n) * yscale)) }, label))));
        x += xdif;
    }
}
