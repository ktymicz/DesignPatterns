...
// sum  = sin + cos
inline double sum_sin_cos(double x) { return cos(x) + sin(x); }
// sum of squares
inline double sum_squares_sin_cos(double x) { return cos(x)*cos(x) + sin(x)*sin(x); }
...
// calculates points for fc (graph function, parabola etc.), 
// it is returning vector<Points>, 
std::vector<Point> calculateGraphPointsF(Fct fc, double r1, double r2, Point orig, int count, double xscale = 25, double yscale = 25);
...
std::string integer_to_string(int d);
...
public:
	//FunctionControler() { std::cout << "Call to Constructor Function Controller\n"; };
	FunctionControler(Fct fc, double r1, double r2, Point orig, int count, double xscale=25, double yscale=25);
	...
	void set_scale(double xscale = 25, double yscale = 25);
	void set_fct(Fct fc);	
};

template <class T>
class TempFunctionControler
	: public Shape
{
private:
	double r[2];
	...
	T t;
public:
	TempFunctionControler(T tp, Fct fc, double r1, double r2, Point orig, int count, double xscale = 25, double yscale = 25);
	void set_t(T tp);
	T& get_t() { return t; };
	void set_range(double r1, double r2);
	//void set_scale(double xscale = 25, double yscale = 25);
	//void set_fct(Fct fc);
	Fct& function() { return f; }
	//friend std::ostream& operator<<(std::ostream& o, T tp);
};

template <class T>
TempFunctionControler<T>::TempFunctionControler(T tp, Fct fc, double r1, double r2, Point orig, int count, double xscale, double yscale)
	: t{ tp }, f{ fc }, r{ r1,r2 }, po{ orig }, c{ count }, xs{ xscale }, ys{ yscale }
{
...
//class Scale
//{
//	Scale(int cordinate_base, int value_base, double vscale)
//		: cbase{ cordinate_base }, vbase{ value_base }, vs{ vscale }
//	int give(int n) { return cbase + vs*n - vbase; }
//	int cbase;
//	int vbase;
//	double vs;
//};
...
template <class T>
void TempFunctionControler<T>::set_t(T tp)
{
	t = tp;
}

template <class T>
void TempFunctionControler<T>::set_range(double r1, double r2)
{
	r[0] = r1;
	r[1] = r2;
	f = calculateGraphPointsF(f, r[0], r[1], po, c, xs, ys);
}
//------------------------------------------------------------------------------
class Bars
{
...
public:
	Bars(std::initializer_list<double> list, Point p, double xscale = 30, double yscale = 25);
	std::vector< std::pair<Rectangle, Text>> pairs;
	void attach_owned(Simple_window& w); // attach own shapes to window
};
...
class Bars2
{
	...
	double xs, ys;
	std::pair<std::string, std::string>& pair_double_to_string(std::pair<double, double>& p);
public:
	Bars2(std::vector<std::pair<double, double>>& vp, Point p, double xscale = 30, double yscale = 25);
	//Bars2(std::vector<std::pair<int, int>>& vp, Point p, double xscale = 30, double yscale = 25);
	std::vector<Rectangle> v_rectangles;
	std::vector<std::pair<Text, Text>> vp_texts;
	void attach_owned(Simple_window& w); // attach own shapes to window
};
...
public:
	Bar(std::initializer_list<double> list, Point p, int width, int height, double xscale = 25, double yscale = 25);
	std::vector< std::pair<Rectangle, Text>> pairs;
	void attach_owned(Simple_window& w); // attach own shapes to window
...
public:
	Barv(std::initializer_list<double> list, Point p, int width, int height, double xscale=25, double yscale=25);
	std::vector< std::unique_ptr< std::pair<Rectangle,Text>>> pairs;
};
//---------------------------------
